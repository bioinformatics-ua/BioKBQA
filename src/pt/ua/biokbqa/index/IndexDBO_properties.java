...
public class IndexDBO_properties {
	...
	public ArrayList<String> search(final String object) {
		...
		try {
			...
			for (ScoreDoc hit : hits) {
				...
				System.out
						.println(object + "->" + hitDoc.get(FIELD_NAME_SUBJECT) + ", " + hitDoc.get(FIELD_NAME_OBJECT));
	...
	private void index() {
		try {
			Model model = RDFDataMgr.loadModel("resources/properties.ttl");
	...
	private void addDocumentToIndex(final Resource resource, final String predicate, final RDFNode next)
			throws Exception {
